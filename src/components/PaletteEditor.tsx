...
export default function PaletteEditor({ baseColors }: PaletteEditorProps) {
  ...
  const scaleFn = oklch
    ? scale([
        { ...oklch, l: 0.95 } as typeof oklch,
        oklch,
        { ...oklch, l: 0.2 } as typeof oklch,
      ])
    : () => current;
  const steps = oklch ? Array.from({ length: 10 }, (_, i) => formatHex(scaleFn(i / 9))) : [current];
          ...
          {steps.map((c, i) => {
            ...
            return (
              <div
                ...
                style={{ backgroundColor: c }}
              >
                <span className="text-xs font-mono text-white drop-shadow">{c}</span>
