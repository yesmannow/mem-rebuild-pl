    ...
    <>
        ...
        {isMenuOpen && (
          ...
          >
            <motion.div
              ...
              className="absolute inset-x-0 bottom-0 top-20 bg-black/40 backdrop-blur-2xl rounded-t-3xl border-t border-white/10"
            ...
            >
              ...
              <div className="flex items-center justify-between px-6 py-5 border-b border-white/10">
                ...
                <button
                  onClick={() => setIsMenuOpen(false)}
                  className="p-2.5 rounded-xl bg-white/5 border border-white/10 hover:border-cyan-400/30 text-brand-muted hover:text-cyan-400 transition-all"
              ...
              <div className="px-6 py-3 border-b border-white/10 flex items-center gap-3">
                ...
                <div className="relative group">
                  ...
                  <div className="absolute left-full ml-2 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-50">
                    <div className="px-2 py-1 text-xs bg-black/60 backdrop-blur-md border border-white/10 rounded text-white">
                ...
                <button
                  onClick={openPalette}
                  className="px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-brand-muted hover:text-cyan-400 hover:border-cyan-400/30 transition"
                  ...
                  <div className="flex items-center gap-2 min-w-max">
                    {[
                      ...
                      <Link
                        ...
                        to={q.path}
                        onClick={handleMenuItemClick}
                        className="px-3 py-1.5 text-sm rounded-full border border-white/10 bg-white/5 text-brand-muted hover:text-cyan-400 hover:border-cyan-400/30 whitespace-nowrap"
                      >
                        {q.label}
                      </Link>
                    ))}
                      ...
                      >
                        <Link
                          ...
                          className={`flex flex-col items-center justify-center p-4 sm:p-5 rounded-2xl border transition-all duration-300 min-h-[110px] sm:min-h-[130px] ${
                            active
                              ? 'bg-cyan-400/20 border-cyan-400/50 shadow-lg shadow-cyan-400/20'
                              : 'bg-white/5 border-white/10 hover:bg-white/10 hover:border-cyan-400/30 active:scale-95'
                          }`}
                        >
                          <div className={`mb-3 p-3 rounded-xl ${
                            ...
                            <Icon
                              size={28}
                              className={active ? 'text-cyan-400' : 'text-brand-muted'}
                              style={{
                                filter: active ? 'drop-shadow(0 0 8px rgba(0, 242, 255, 0.6))' : 'none',
                              }}
                            />
                          </div>
      ...
      >
        <motion.div
          ...
          className="relative bg-black/40 backdrop-blur-2xl rounded-2xl border border-white/10 shadow-2xl"
          style={{
            boxShadow:
              '0 8px 32px rgba(0, 0, 0, 0.5), 0 0 0 1px rgba(0, 242, 255, 0.15), 0 -2px 24px rgba(0, 242, 255, 0.25)',
          }}
        >
          {/* Teal accent line at top */}
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-16 h-1 bg-gradient-to-r from-transparent via-cyan-400 to-transparent rounded-full" />
          ...
          <div className="flex items-center justify-evenly px-2 sm:px-3 py-2.5 sm:py-3">
                ...
                >
                  ...
                  >
                    <Icon
                      ...
                      className={`transition-all duration-300 sm:w-6 sm:h-6 ${
                        active ? 'text-cyan-400' : 'text-brand-muted group-hover:text-cyan-400/70'
                      }`}
                      style={{
                        filter: active ? 'drop-shadow(0 0 8px rgba(0, 242, 255, 0.6))' : 'none',
                      }}
                  ...
                  <span
                    className={`text-[10px] sm:text-[11px] whitespace-nowrap mt-1 sm:mt-1.5 font-semibold transition-colors duration-300 ${
                      active ? 'text-cyan-400' : 'text-brand-muted/70'
                    }`}
                  >
                    {item.label}
                  </span>
            ...
            <motion.button
              ...
              className={`relative flex flex-col items-center justify-center p-2 sm:p-3 rounded-xl transition-all duration-300 min-w-[64px] sm:min-w-[72px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900 ${
                isMenuOpen ? 'bg-cyan-400/20 border border-cyan-400/30' : ''
              }`}
            >
              <Grip
                size={22}
                className={`transition-all duration-300 sm:w-6 sm:h-6 ${
                  isMenuOpen ? 'text-cyan-400' : 'text-brand-muted'
                }`}
                style={{
                  filter: isMenuOpen ? 'drop-shadow(0 0 8px rgba(0, 242, 255, 0.6))' : 'none',
                }}
              />
              <span
                className={`text-[10px] sm:text-[11px] whitespace-nowrap mt-1 sm:mt-1.5 font-semibold transition-colors duration-300 ${
                  isMenuOpen ? 'text-cyan-400' : 'text-brand-muted/70'
                }`}
              >
                More
              </span>
