...
export default function MarkSandbox() {
  ...
  const options: MarkOption[] = [
    ...
    },
    {
      ...
      render: () => (
        <svg width="120" height="120">
          <polygon points="60,10 110,110 10,110" fill="none" stroke="black" strokeWidth="4" />
        </svg>
      ),
    },
    {
      ...
      render: () => (
        <svg width="120" height="120">
          <rect x="20" y="20" width="80" height="80" fill="none" stroke="black" strokeWidth="4" />
        </svg>
      ),
    },
  ];
    ...
    <section className="py-16 bg-slate-50 relative">
      ...
      <div className="flex justify-center mb-8">
        <label className="flex items-center gap-2">
          <input type="checkbox" checked={sketchy} onChange={e => setSketchy(e.target.checked)} />
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div className="p-6 bg-white rounded-lg shadow">
            {sketchy ? <SketchWrapper>{current.render()}</SketchWrapper> : current.render()}
          </div>
        </motion.div>
      </AnimatePresence>
    </section>
...
function SketchWrapper({ children }: { readonly children: React.ReactElement }) {
  return (
    <div className="opacity-80">
      {children}
      <p className="text-xs text-slate-500 text-center mt-2">(Sketchy variant)</p>
    </div>
  );
}
