...
export function AppSection({
  ...
  return (
    <motion.section
      className={cn('relative', variantStyles[variant], paddingStyles[padding], className)}
      initial={{ opacity: 0 }}
      ...
      {...props}
    >
      {container ? <div className="container mx-auto px-4 md:px-6 lg:px-8">{children}</div> : children}
    </motion.section>
  );
}
