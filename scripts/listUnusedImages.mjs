 ...
 * 
 * Usage: 
 *   node scripts/listUnusedImages.mjs           # List unused images
 *   node scripts/listUnusedImages.mjs --move    # Move unused images to archive
 */
...
const getAllImageFiles = () =>
  fg.sync(`${imageDir}/**/*.{png,jpg,jpeg,webp,avif,svg}`);

const getAllProjectFiles = () =>
  srcDirs.flatMap(dir => {
    if (!fs.existsSync(dir)) return [];
    return fg.sync(`${dir}/**/*.{tsx,ts,jsx,js,md,mdx}`);
  });

const isImageUsed = (imagePath, projectFiles) => {
  ...
  if (shouldMove) {
    ...
    unusedImages.forEach(img => {
      try {
        ...
        console.error(`Failed to move ${path.basename(img)}:`, e.message);
      }
    });
    ...
    console.log(`   node scripts/listUnusedImages.mjs --move`);
  }
};
