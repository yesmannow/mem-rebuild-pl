#!/usr/bin/env node
// Node-only trace script for PowerShell (creates reports/CE_TRACE_SUMMARY.md and UNBLOCK-TODO.md)
const fs = require("fs");
...
try {
  append(execSync('git grep -n "customElements.define" || true', { encoding: "utf8" }));
} catch (e) { append("(git grep failed)"); }
append("");
append("=== git grep mce-autosize-textarea ===");
try {
  append(execSync('git grep -n "mce-autosize-textarea" || true', { encoding: "utf8" }));
} catch (e) { append("(git grep failed)"); }
...
try {
  ...
  append("(node_modules scan failed or node_modules missing)");
}
